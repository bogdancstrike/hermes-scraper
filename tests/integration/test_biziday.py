...
Output: tests/output/test_biziday/
"""
...
MAX_PAGES = 1
MAX_ARTICLES = 5
OUTPUT_DIR = Path(__file__).parent.parent / "output" / "test_biziday"
...
def articles() -> list[dict]:
    """Run the full app pipeline against biziday.ro once for the whole module."""
    results = asyncio.run(
...
def test_articles_have_urls_on_correct_domain(articles):
    """Every article must have a non-empty URL on the biziday.ro domain."""
    if not articles:
        pytest.skip("No articles")
    for a in articles:
...
def test_articles_tagged_with_correct_domain(articles):
    """Articles must be tagged with domain=biziday.ro."""
    if not articles:
        pytest.skip("No articles")
    for a in articles:
...
def test_output_files_created(articles):
    ...
    assert list(OUTPUT_DIR.glob("biziday_ro_*.json")), "JSON output file not created"
    assert list(OUTPUT_DIR.glob("biziday_ro_*.csv")), "CSV output file not created"
