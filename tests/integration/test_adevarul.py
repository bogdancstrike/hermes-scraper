...
Output: tests/output/test_adevarul/
"""
...
MAX_PAGES = 1
MAX_ARTICLES = 5
OUTPUT_DIR = Path(__file__).parent.parent / "output" / "test_adevarul"
...
def articles() -> list[dict]:
    """Run the full app pipeline against adevarul.ro once for the whole module."""
    results = asyncio.run(
...
def test_articles_have_urls_on_correct_domain(articles):
    """Every article must have a non-empty URL on the adevarul.ro domain."""
    if not articles:
        pytest.skip("No articles")
    for a in articles:
...
def test_articles_tagged_with_correct_domain(articles):
    """Articles must be tagged with domain=adevarul.ro."""
    if not articles:
        pytest.skip("No articles")
    for a in articles:
...
def test_articles_are_romanian(articles):
    """adevarul.ro articles with detected language must be Romanian."""
    if not articles:
        pytest.skip("No articles")
    with_lang = [a for a in articles if a.get("language")]
    if not with_lang:
        pytest.skip("No articles with detected language")
    romanian = [a for a in with_lang if a["language"] == "ro"]
    ratio = len(romanian) / len(with_lang)
    assert ratio >= 0.5, (
        f"Only {len(romanian)}/{len(with_lang)} articles detected as Romanian"
    )
...
def test_output_files_created(articles):
    ...
    assert list(OUTPUT_DIR.glob("adevarul_ro_*.json")), "JSON output file not created"
    assert list(OUTPUT_DIR.glob("adevarul_ro_*.csv")), "CSV output file not created"
